...
def Geom4(cur):
	...
	for start_board in [
		...
		desc = '''Квадрат 5×5 разбили на 10 фигурок и в одной из клеток каждой фигуры 
				записали её периметр. Восстановите разбиение. Используйте 10 разных цветов справа 
...
def CombiRepublic4(cur):
	...
	for N, A in [
			...
			Палиндром это число из нескольких цифр, стоящих рядом, которое читается одинаково слева направо и справа налево."""
			cont = {
				'correct': A,
			}
	...
	for amount, transfers in [
			...
			desc = f"""В стране {amount} городов, из каждого из которых есть не более четырёх авиалиний в другие города. 
					Известно, что из любого города можно долететь в любой другой, сделав не более трёх пересадок. 
					Соедините города авиалиниями так, чтобы выполнялись все условия и авиалиний было как можно меньше."""
			cont = {
				'amount': amount,
				'transfers': transfers,

...
def ОстровЛжецов(cur):
	...
	задача = add_problem(cur, "Остров Лжецов", 4, 'integer', "Лжецы и хитрецы", "Возьмите какого-нибудь правдолюба (П), рядом с ним по условию должны сидеть хитрец (Х) и лжец (Л).  Докажите, что рядом с лжецом сидит ещё один правдолюб.")
	for N, A in [(20, 8), (30, 12), (50, 20), (60, 24)]:
		desc = f"За круглым столом сидят {N} человек, каждый из которых либо правдолюб (всегда говорит правду), либо лжец (всегда лжёт), либо хитрец (из любых двух его утверждений одно истина, а другое — ложь). Каждый из сидящих заявил «рядом со мной сидит лжец» и «рядом со мной сидит хитрец». Какое наименьшее число хитрецов может быть за столом?"
		cont = {
			'correct': A,
		}
		add_variant(cur, задача, desc, json.dumps(cont))
 ...
 #
	...
	задача = add_problem(cur, "Остров Лжецов", 2, 'radio', "Дело о краже кораллов", "Предположите по очереди, что один из них сказал правду. Посмотрите, получается противоречие или нет. Результаты удобно отражать в таблице.")
	for a, b, c, A in [

...
def IslandOfLiars4(cur):
	...
	problems_list = add_problem_to_list(problems_list, cur, "Друзья на физкультуре", 2, 'friends_on_phys', "Соседи лжецы", hint="Про одного из мальчиков известно, что рядом с ним не стоят два других мальчика. Что в таком случае можно сказать о его местоположении?")
...
def Chiselburg4(cur):
	...
	for start_board, cows_and_bulls, correct in [
		...
		В каждой из следующих строк написаны четыре цифры и указано, что именно угадано: бык ставится за каждую цифру, 
		которая стоит в нужном столбце, а корова – за каждую цифру, которая есть в этих четырёх столбцах искомой комбинации, 

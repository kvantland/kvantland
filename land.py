...
def show_land(db):
	...
	yield f'<p>Обратите внимание, что некоторые задачи интерактивны. В них требуется произвести действия, которые описаны в условии, чтобы получить нужный результат. Читайте условия внимательно! Для решения задач вам понадобится компьютерная мышь, чтобы перетаскивать или выделять объекты. Если возникла техническая проблема, то можно написать в техподдержку <a href="mailto:{config["contacts"]["support_email"]}"> support@kvantland.com </a> с описанием проблемы и скриншотом компьютера.'

...
def show_land(db):
	...
	yield '</div>'
	yield '<div class="rules_wrapper">'
	yield '<div class="span_position"><span class="span_text">В начале игры каждому дается 10 квантиков, которые можно тратить на подсказки к задачам. Для того чтобы получить задачу, нужно выбрать одну из монет в соответствующем городе или области (кликнуть на монету). На монете указывается количество квантиков, которое даются за её правильное решение. <br/><br/></span><span>Есть задачи проще (1 или 2 квантика за решение) и сложнее (3 или 4 квантика за решение).</span></div>'
	yield '</div>'
	yield '<div class="span_wrapper"><span class="span_text">Можно свободно возвращаться к карте города или страны. Но если вы уже давали ответ на задачу, то задача становится неактивной и пройти её повторно нельзя. <br/><br/></span><span>Поэтому не торопитесь и внимательно проверяйте, прежде чем отправить ответ. </span></div>'
	...
	yield '<div class="span_wrapper span_text">Выберите время в течение месяца, чтобы вас ничего не отвлекало. Итоги соревнования подводятся по числу квантиков, которое у вас на счету к концу игры. Это число всегда отображается в правом верхнем углу экрана. Удачи!</div>'
	yield '</div>'

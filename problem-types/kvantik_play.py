def entry_form(data, kwargs):
	...
	for y in range(0, 1):
		for x in range(0, in_row):	
			yield f'<text class="field color{x}" x="{x * side}" y="{(y + 1) * side}" width="{inner_side}" height="{inner_side}">{data['word'][x]}</text>'
	for y in range(1, 2):
		for x in range(0, in_row):	
			...
			yield f'<input class="fieldinput" width="{inner_side}" height="{inner_side}" type="number" maxlength="1">'
			yield f'</div>'

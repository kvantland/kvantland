def entry_form(data, kwargs):
	...
	basement_width = 250
	basement_hight = 80
	base_inside_yr = 100 * 0.4
	base_inside_xr = 200 * 0.4
	coin_size = 80
	coin_pad = 10
	drag_pad = 30
	svg_width = (plank_length + cup_xr * 2) * scale_1
	svg_height = (base_plank_width / 4 + base_plank_height + basement_hight + basement_width / 4) * scale_1 + drag_pad + coin_size
	...
	yield '<stop offset="0%" stop-color="#7B6F01" />'
	yield '<stop offset="100%" stop-color="#373200" />'
	...
	yield f"""<path class="base_outside" transform="translate({-basement_width / 2} {base_plank_height - base_plank_up})"
				d="M 0 0 
				V {basement_hight}
				A {basement_width / 2} {basement_width / 4} 0 0 0 {basement_width} {basement_hight}
				V 0
				H 0
				Z" />"""
	...
	yield f"""<path class="base_plank"
				...
				d="M 0 0
	 			V {base_plank_height}
	 			A {base_plank_width / 2} {base_plank_width / 4} 0 0 0 {base_plank_width} {base_plank_height}
	 			V 0
	 			A {base_plank_width / 2} {base_plank_width / 4} 0 0 0 0 0"
	 			Z" />"""
	...
	yield '</g>'
	yield '</g>'
	yield f'<g class="drag_zone" transform="translate(0 {svg_height - coin_size})">'
	for i in range(1, 6):
		yield f'<g class="coin num_{i}" title="{i}" transform="translate({(coin_size + coin_pad) * (i - 1)} 0)">'

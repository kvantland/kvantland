...
def render_lamp_array(w: int, h: int, extra_attrs):
	...
	for y in range(h):
		for x in range(w):
			yield f'<rect {extra_attrs(x, y)} x="{off + x}em" y="{off + y}em" width="1em" height="1em" />'
	yield '</svg>'
...
def entry_form(data, kwargs):
	...
	yield from render_lamp_array(w, h, lambda x, y: f'id="lamp_{x}_{y}" class="lamp lamp_on"')
	yield '</div>'
	yield '<div class="lamp_bar">'
	for k, switch in enumerate(switches):
		yield '<div>'
		yield from render_lamp_array(w, h, lambda x, y: 'class="lamp lamp_on"' if switch[y][x] else 'class="lamp"')

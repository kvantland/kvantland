...
def get_policy_pars():
	policy_pars = [
		{   
			...
			'desc':  [
				"""Настоящая Политика конфиденциальности составлена в соответствии с требованиями Федерального закона
				...
				"""Опе­ра­тор пер­со­наль­ных дан­ных ООО "Квантик", ИНН 7728344440 (далее – Опе­ра­тор) ста­вит важ­ней­шей 
				...
				личную и се­мей­ную тайну. """, 
				
				"""Настоящая Политика конфиденциальности Оператора применяется ко всей информации, которую Оператор 
				может получить о посетителях веб-сайта, размещенного в сети «Интернет» по адресу: 
				<a href="https://www.kvantland.com"> https://www.kvantland.com</a>"""
				]
		},		
		...
		{   
			...
			'desc':  [
				...
				"""Обезличивание персональных данных (уничтожение) – действия, в результате которых становится 
				невозможным определить принадлежность персональных данных конкретному субъекту персональных 
				данных;""", 
				
				"""Все действия по использованию Сайта, включая отправку любых сообщений, содержащих персональные 
				данные Пользователя через формы на сайте, возможны исключительно при наличии его свободного согласия 
				на обработку персональных данных в порядке, описанном в Политике.""", 
				
				"""Отправляя свои персональные данные в форме обратной связи на сайте 
				<a href="https://www.kvantland.com"> https://www.kvantland.com</a>, 
				Пользователь выражает согласие с условиями обработки персональных данных, предусмотренных 
				Политикой, и подтверждает, что: – он ознакомлен с Политикой, и данный документ ему понятен.""", 
				
				f"""Согласие Пользователя на обработку персональных данных может быть отозвано путем направления 
				заявления по электронному адресу <a href="mailto:{config["contacts"]["support_email"]}">
				support@kvantland.com</a>. Для осуществления отзыва согласия 
				Пользователь обязан указать фамилию, имя и адрес электронной почты.""", 
				
				"""Согласие на обработку персональных данных действует до достижения цели получения согласия, если иной 
				срок обработки не следует из требований законодательства РФ.""", 
				
				"""При несогласии с любым указанным в Политике положением Пользователь обязан отказаться от совершения 
		...
		{   
			...
			'desc':  [
				"""Оператор собирает данные, полученные от пользователей в процессе их регистрации и/или использования 
				...
				безопасности и устранения неполадок, а также предоставления поддержки.""", 
				
				"""Цели использования Cookie-файлов («куки»-файлов). Файлы сookie представляют собой небольшие текстовые 
		...
		
		{
			...
			'desc':  [
				"""Сведениями, составляющими персональные данные, является любая информация, относящаяся к прямо или 
				косвенно определенному или определяемому физическому лицу (субъекту персональных данных).""", 
				
				"""Вы предоставляете некоторые из этих данных непосредственно, например, когда создаете учетную запись 
				...
				награждении победителей.""", 
				
				"""В объем собираемых данных могут входить следующие: фамилия, имя, наименование и номер школы, номер 
		...
		{   
			...
			'desc':  [
				"""Обработка персональных данных осуществляется на основе принципов законности целей и способов 
				...
				"""Персональные данные Пользователей не передаются каким-либо третьим лицам. Предоставление 
				персональных данных пользователей по запросу государственных органов или органов местного самоуправ
				ления осуществляется в порядке, предусмотренном законодательством.""", 
				
				f"""Персональные данные пользователя уничтожаются при: удалении Оператором Аккаунта пользователя по его 
				запросу на электронную почту <a href="mailto:{config["contacts"]["support_email"]}">support@kvantland.com</a>.""", 
				
				"""Оператор оставляет за собой право в автоматическом режиме удалять данные пользователя по истечении 
		...
		
		{
			...
			'desc':  [
				"""Оператор принимает технические и организационно-правовые меры в целях обеспечения защиты 
				...
				"""Оператор определяет угрозы безопасности персональных данных при их обработке, формирует на их основе 
				...
				"""Оператор проводит технические мероприятия, направленные на предотвращение несанкционированного 
		...
		
		{
			...
			'desc':  [
				"""Оператор персональных данных вправе предоставлять персональные данные субъектов третьим лицам, если 
				...
				"""Субъект персональных данных имеет право требовать уточнения своих персональных данных, их 
		...
		{   
			...
			'desc':  [
				"""Настоящая Политика подлежит изменению или дополнению в случае появления новых законодательных 
				актов и специальных нормативных документов по обработке и защите персональных данных.""", 
				
				"""Актуальная версия Политики расположена в свободном доступе в сети Интернет по адресу
				<a href="https://www.kvantland.com/policy"> https://kvantland.com/policy</a>.""",
				
				f"""Все вопросы Пользователей по поводу настоящей Политики, а также запросы на уточнение, блокирование 

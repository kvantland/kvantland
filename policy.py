...
def get_policy_pars():
	policy_pars = [
		{   'header': "Общие положения",
            'desc':  ["""Настоящая Политика конфиденциальности составлена в соответствии с требованиями Федерального закона
				от 27.07.2006 № 152-ФЗ «О персональных данных» и определяет порядок обработки персональных данных
				Оператором и меры по обеспечению безопасности персональных данных. """, """Опе­ра­тор пер­со­наль­ных дан­ных ООО "Квантик", ИНН 7728344440 (далее – Опе­ра­тор) ста­вит важ­ней­шей 
				...
				личную и се­мей­ную тайну. """, """Настоящая Политика конфиденциальности Оператора применяется ко всей информации, которую Оператор 
				может получить о посетителях веб-сайта, размещенного в сети «Интернет» по адресу: <a href="https://www.kvantland.com"> https://www.kvantland.com</a>"""]},		
		{   'header': "Термины и понятия",
            'desc':  ["""Персональные данные - любая информация, относящаяся к прямо или косвенно определенному 
				или определяемому физическому лицу (субъекту персональных данных);""", """Предоставление персональных данных – действия, направленные на раскрытие персональных данных 
				определенному лицу или определенному кругу лиц;""", """Настоящая Политика конфиденциальности Оператора применяется ко всей информации, которую Оператор 
				может получить о посетителях веб-сайта, размещенного в сети «Интернет» по адресу: <a href="https://www.kvantland.com"> https://
				www.kvantland.com</a>""", """Обработка персональных данных – любое действие (операция) или совокупность действий (операций), 
				...
				обезличивание, блокирование, удаление, уничтожение персональных данных;""", """Автоматизированная обработка персональных данных – обработка персональных данных с помощью средств 
				вычислительной техники;""", """Обезличивание персональных данных (уничтожение) – действия, в результате которых становится 
				невозможным определить принадлежность персональных данных конкретному субъекту персональных 
				данных;""", """Все действия по использованию Сайта, включая отправку любых сообщений, содержащих персональные 
				данные Пользователя через формы на сайте, возможны исключительно при наличии его свободного согласия 
				на обработку персональных данных в порядке, описанном в Политике.""", """Отправляя свои персональные данные в форме обратной связи на сайте <a href="https://www.kvantland.com"> https://www.kvantland.com</a>, 
				Пользователь выражает согласие с условиями обработки персональных данных, предусмотренных 
				Политикой, и подтверждает, что: – он ознакомлен с Политикой, и данный документ ему понятен.""", """Согласие Пользователя на обработку персональных данных может быть отозвано путем направления 
				...
				Пользователь обязан указать фамилию, имя и адрес электронной почты.""", """Согласие на обработку персональных данных действует до достижения цели получения согласия, если иной 
				срок обработки не следует из требований законодательства РФ.""", """При несогласии с любым указанным в Политике положением Пользователь обязан отказаться от совершения 
				действий на Сайте, для которых требуется использование Оператором его персональных данных."""]},
		{   'header': "Цели обработки персональных данных",
            'desc':  ["""Оператор собирает данные, полученные от пользователей в процессе их регистрации и/или использования 
				...
				безопасности и устранения неполадок, а также предоставления поддержки.""", """Цели использования Cookie-файлов («куки»-файлов). Файлы сookie представляют собой небольшие текстовые 
				...
				ми и запрете их использования."""]},
		{   'header': "Состав персональных данных",
            'desc':  ["""Сведениями, составляющими персональные данные, является любая информация, относящаяся к прямо или 
				косвенно определенному или определяемому физическому лицу (субъекту персональных данных).""", """Вы предоставляете некоторые из этих данных непосредственно, например, когда создаете учетную запись 
				...
				награждении победителей.""", """В объем собираемых данных могут входить следующие: фамилия, имя, наименование и номер школы, номер 
				...
				и контролируете посредством Сайта."""]},
		{   'header': "Порядок обработки персональных данных",
            'desc':  ["""Обработка персональных данных осуществляется на основе принципов законности целей и способов 
				...
				обработки персональных данных.""", """Персональные данные пользователей хранятся исключительно на электронных носителях и обрабатываются 
				с использованием автоматизированных систем, за исключением случаев, когда неавтоматизированная 
				обработка персональных данных необходима в связи с исполнением требований законодательства.""", """Персональные данные Пользователей не передаются каким-либо третьим лицам. Предоставление 
				персональных данных пользователей по запросу государственных органов или органов местного самоуправ
				ления осуществляется в порядке, предусмотренном законодательством.""", """Персональные данные пользователя уничтожаются при: удалении Оператором Аккаунта пользователя по его 
				запросу на электронную почту <a href="mailto:{config["contacts"]["support_email"]}">support@kvantland.com</a>.""", """Оператор оставляет за собой право в автоматическом режиме удалять данные пользователя по истечении 
				одного календарного года со дня последнего посещения пользователем Сайта Оператора."""]},
		{   'header': "Меры по защите персональных данных",
            'desc':  ["""Оператор принимает технические и организационно-правовые меры в целях обеспечения защиты 
				персональных данных Пользователя от неправомерного или случайного доступа к ним, уничтожения, 
				изменения, блокирования, копирования, распространения, а также от иных неправомерных действий""", """Оператор определяет угрозы безопасности персональных данных при их обработке, формирует на их основе 
				...
				осуществляет установку и ввод в эксплуатацию средств защиты информации.""", """Оператор проводит технические мероприятия, направленные на предотвращение несанкционированного 
				...
				постоянный контроль за обеспечением уровня защищенности персональных данных.""", """Для разработки и осуществления мероприятий по обеспечению безопасности персональных данных при их 
				...
				предоставления Пользователю доступа к его использованию."""]},
		{   'header': "Права и обязанности оператора и субъекта персональных данных",
            'desc':  ["""Оператор персональных данных вправе предоставлять персональные данные субъектов третьим лицам, если 
				...
				законодательством.""", """Субъект персональных данных имеет право требовать уточнения своих персональных данных, их 
		...
		{   'header': "Заключительные положения",
            'desc':  ["""Настоящая Политика подлежит изменению или дополнению в случае появления новых законодательных 
				актов и специальных нормативных документов по обработке и защите персональных данных.""", """Актуальная версия Политики расположена в свободном доступе в сети Интернет по адресу <a href="https://www.kvantland.com/policy"> https://
				kvantland.com/policy</a>.""", """Все вопросы Пользователей по поводу настоящей Политики, а также запросы на уточнение, блокирование 
				или уничтожение их персональных данных они вправе направлять оператору по адресу электронной почты: 
				<a href="mailto:{config["contacts"]["support_email"]}">support@kvantland.com</a>."""]},							
    ]
	return json.dumps(policy_pars)

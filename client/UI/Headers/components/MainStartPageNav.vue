...
export default {
    data() {
        return {
            menuItems: [
                {'id': "TournamentInfoSection", 'title': "Турниры"},
                {'id': "MainInfoSection", 'title': "О нас"},
                {'id': "KvantlandTeamInfoSection", 'title': "О команде"},
                {'id': "ContactsSection", 'title': "Наши контакты"}
            ]
        }
    },
...
.menu {
	...
	align-items: center; 
	gap: max(10px, min(40px, 2.6vw)); 
	display: inline-flex;
}
.menu .menu_item {
	color: black; 
	font-size: max(10px, min(16px, 1.25vw)); 
...
.menu .menu_item:active {
	background: rgba(26.35, 88.38, 102, 0.20);
}
@media(hover:hover) {

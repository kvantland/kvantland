...
def show_town(db, town):
	...
	db.execute('select вариант, положение, баллы, название from ДоступнаяЗадача join Вариант using (вариант) join Задача using (задача) where город = %s', (town, ))
	
	cur_position = []
	for variant, position, points, name in db.fetchall():
		try:
			x, y = position
